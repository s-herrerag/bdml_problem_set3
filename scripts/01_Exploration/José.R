...
confusion_matrix <- conf_mat(test_data, truth = Pobre, estimate = predicciones_tree )

accuracy <- accuracy(test_data, truth = Pobre, estimate = predicciones_tree)
...
class_tree <- train(Pobre ~ ., data = train_data, method = "rpart", trainControl = control, tuneLength = 10)